import * as THREE from 'three';

const CANVAS_W = 800;
const CANVAS_H = 600;
const PIPE_W   = 64;
const CAP_H    = 22;

// ─── Emoji sprite factory ─────────────────────────────────────────────────────
function makeEmojiSprite(emoji, color) {
  const size = 128;
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext('2d');

  // Colored circle background
  ctx.fillStyle = '#' + color.toString(16).padStart(6, '0');
  ctx.beginPath();
  ctx.arc(size / 2, size / 2, size / 2 - 4, 0, Math.PI * 2);
  ctx.fill();

  // White ring
  ctx.strokeStyle = 'rgba(255,255,255,0.6)';
  ctx.lineWidth = 4;
  ctx.stroke();

  // Emoji
  ctx.font = `${size * 0.52}px serif`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText(emoji, size / 2, size / 2 + 4);

  const texture = new THREE.CanvasTexture(canvas);
  const material = new THREE.SpriteMaterial({ map: texture, depthTest: false });
  const sprite = new THREE.Sprite(material);
  sprite.scale.set(52, 52, 1);
  return sprite;
}

// ─── Pipe mesh factory ────────────────────────────────────────────────────────
function makePipePair(scene) {
  const bodyMat = new THREE.MeshBasicMaterial({ color: 0x3CB043 });
  const capMat  = new THREE.MeshBasicMaterial({ color: 0x228B22 });
  const bodyGeo = new THREE.BoxGeometry(PIPE_W, 1, 40);
  const capGeo  = new THREE.BoxGeometry(PIPE_W + 14, CAP_H, 46);

  const top       = new THREE.Mesh(bodyGeo, bodyMat);
  const topCap    = new THREE.Mesh(capGeo, capMat);
  const bottom    = new THREE.Mesh(bodyGeo, bodyMat);
  const bottomCap = new THREE.Mesh(capGeo, capMat);

  scene.add(top);    scene.add(topCap);
  scene.add(bottom); scene.add(bottomCap);
  return { top, topCap, bottom, bottomCap };
}

// ─── Background ───────────────────────────────────────────────────────────────
function buildBackground(scene) {
  const sky = new THREE.Mesh(
    new THREE.PlaneGeometry(CANVAS_W * 2, CANVAS_H * 2),
    new THREE.MeshBasicMaterial({ color: 0x87CEEB })
  );
  sky.position.set(CANVAS_W / 2, CANVAS_H / 2, -80);
  scene.add(sky);

  const ground = new THREE.Mesh(
    new THREE.BoxGeometry(CANVAS_W * 2, 40, 50),
    new THREE.MeshBasicMaterial({ color: 0x8B6914 })
  );
  ground.position.set(CANVAS_W / 2, -10, -20);
  scene.add(ground);

  const grass = new THREE.Mesh(
    new THREE.BoxGeometry(CANVAS_W * 2, 14, 52),
    new THREE.MeshBasicMaterial({ color: 0x5DBB3F })
  );
  grass.position.set(CANVAS_W / 2, 14, -20);
  scene.add(grass);

  const clouds = [];
  for (let i = 0; i < 7; i++) {
    const cloud = makeCloud();
    cloud.position.set(
      Math.random() * CANVAS_W,
      CANVAS_H * 0.55 + Math.random() * CANVAS_H * 0.35,
      -40 - Math.random() * 30
    );
    scene.add(cloud);
    clouds.push({ mesh: cloud, speed: 0.2 + Math.random() * 0.3 });
  }
  return clouds;
}

function makeCloud() {
  const group = new THREE.Group();
  const mat = new THREE.MeshBasicMaterial({ color: 0xF0F8FF });
  [[0,0,0,60,32,30],[36,10,0,48,26,28],[-34,8,0,44,24,26],[20,-10,0,38,22,24]].forEach(([x,y,z,w,h,d]) => {
    const m = new THREE.Mesh(new THREE.BoxGeometry(w,h,d), mat);
    m.position.set(x,y,z);
    group.add(m);
  });
  return group;
}

// ─── FlappyGame class ─────────────────────────────────────────────────────────
export class FlappyGame {
  constructor(canvas, onFlap, initialPlayers) {
    this.canvas     = canvas;
    this.onFlap     = onFlap;
    this.birds      = {};
    this.pipeMeshes = {};
    this.pipeData   = [];
    this.clouds     = [];
    this.animId     = null;
    this.destroyed  = false;

    this._initRenderer();
    this._initScene();
    this._initPlayers(initialPlayers);
    this._bindInput();
  }

  _initRenderer() {
    const { canvas } = this;
    this.renderer = new THREE.WebGLRenderer({ canvas, antialias: true });
    this.renderer.setPixelRatio(Math.min(devicePixelRatio, 2));
    this._resize();
    this._onResize = () => this._resize();
    window.addEventListener('resize', this._onResize);
  }

  _resize() {
    const w = window.innerWidth;
    const h = window.innerHeight;
    this.renderer.setSize(w, h);

    const aspect = w / h;
    const gameAspect = CANVAS_W / CANVAS_H;
    let left, right, top, bottom;

    if (aspect > gameAspect) {
      const extra = (aspect * CANVAS_H - CANVAS_W) / 2;
      left = -extra; right = CANVAS_W + extra; top = CANVAS_H; bottom = 0;
    } else {
      const extra = (CANVAS_W / aspect - CANVAS_H) / 2;
      left = 0; right = CANVAS_W; top = CANVAS_H + extra; bottom = -extra;
    }

    if (this.camera) {
      this.camera.left = left; this.camera.right = right;
      this.camera.top  = top;  this.camera.bottom = bottom;
      this.camera.updateProjectionMatrix();
    } else {
      this._camParams = { left, right, top, bottom };
    }
  }

  _initScene() {
    const { left, right, top, bottom } = this._camParams || { left: 0, right: CANVAS_W, top: CANVAS_H, bottom: 0 };
    this.camera = new THREE.OrthographicCamera(left, right, top, bottom, -1000, 1000);
    this.camera.position.set(0, 0, 500);
    this.camera.lookAt(0, 0, 0);

    this.scene = new THREE.Scene();
    this.scene.background = new THREE.Color(0x87CEEB);

    this.clouds = buildBackground(this.scene);
  }

  _initPlayers(players) {
    for (const id in players) {
      const p = players[id];
      const sprite = makeEmojiSprite(p.emoji || '🐦', p.color);
      sprite.position.set(p.x, CANVAS_H - p.y, 1);
      this.scene.add(sprite);
      this.birds[id] = { sprite, targetX: p.x, targetY: p.y, alive: p.alive, vy: 0 };
    }
  }

  _bindInput() {
    this._onKey = (e) => {
      if (e.code === 'Space' || e.code === 'ArrowUp') {
        e.preventDefault();
        this.onFlap();
      }
    };
    this._onClick = () => this.onFlap();
    window.addEventListener('keydown', this._onKey);
    this.canvas.addEventListener('click', this._onClick);
    this.canvas.addEventListener('touchstart', this._onClick, { passive: true });
  }

  start() {
    this._loop();
  }

  _loop() {
    if (this.destroyed) return;
    this.animId = requestAnimationFrame(() => this._loop());

    const now = performance.now() * 0.001;

    for (const c of this.clouds) {
      c.mesh.position.x -= c.speed;
      if (c.mesh.position.x < -100) c.mesh.position.x = CANVAS_W + 100;
    }

    for (const id in this.birds) {
      const b = this.birds[id];
      const threeY = CANVAS_H - b.targetY;
      b.sprite.position.y += (threeY - b.sprite.position.y) * 0.25;
      b.sprite.position.x += (b.targetX - b.sprite.position.x) * 0.25;

      // Tilt sprite based on vertical velocity (positive vy = falling = tilt nose down)
      if (b.alive) {
        b.sprite.material.rotation = Math.max(-0.6, Math.min(0.5, b.vy * -0.05));
      }
    }

    this.renderer.render(this.scene, this.camera);
  }

  updateState(pipes, players) {
    for (const id in players) {
      const p = players[id];
      if (!this.birds[id]) {
        const sprite = makeEmojiSprite(p.emoji || '🐦', p.color);
        sprite.position.set(p.x, CANVAS_H - p.y, 1);
        this.scene.add(sprite);
        this.birds[id] = { sprite, targetX: p.x, targetY: p.y, alive: p.alive, vy: p.vy || 0 };
      } else {
        this.birds[id].targetX = p.x;
        this.birds[id].targetY = p.y;
        this.birds[id].alive   = p.alive;
        this.birds[id].vy      = p.vy || 0;
        if (!p.alive) {
          this.birds[id].sprite.material.rotation = -Math.PI / 2;
        }
      }
    }

    this.pipeData = pipes;

    const neededCount = pipes.length;
    const existingKeys = Object.keys(this.pipeMeshes).map(Number);
    for (const k of existingKeys) {
      if (k >= neededCount) {
        const m = this.pipeMeshes[k];
        this.scene.remove(m.top);    this.scene.remove(m.topCap);
        this.scene.remove(m.bottom); this.scene.remove(m.bottomCap);
        delete this.pipeMeshes[k];
      }
    }

    for (let i = 0; i < pipes.length; i++) {
      const pipe = pipes[i];
      const topHeight    = pipe.gapY;                          // top pipe height
      const bottomHeight = CANVAS_H - (pipe.gapY + pipe.gap); // bottom pipe height
      const cx = pipe.x + PIPE_W / 2;

      if (!this.pipeMeshes[i]) {
        this.pipeMeshes[i] = makePipePair(this.scene);
      }

      const { top, topCap, bottom, bottomCap } = this.pipeMeshes[i];

      // Top pipe hangs from screen top (Three.js Y=CANVAS_H) downward
      if (topHeight > 0) {
        top.visible = topCap.visible = true;
        top.scale.y = topHeight;
        top.position.set(cx, CANVAS_H - topHeight / 2, 0);
        // Cap sits at the pipe's open (bottom) end — above the gap, NOT into it
        topCap.position.set(cx, CANVAS_H - topHeight + CAP_H / 2, 0);
      } else {
        top.visible = topCap.visible = false;
      }

      // Bottom pipe rises from screen bottom (Three.js Y=0) upward
      if (bottomHeight > 0) {
        bottom.visible = bottomCap.visible = true;
        bottom.scale.y = bottomHeight;
        bottom.position.set(cx, bottomHeight / 2, 0);
        // Cap sits at the pipe's open (top) end — below the gap, NOT into it
        bottomCap.position.set(cx, bottomHeight - CAP_H / 2, 0);
      } else {
        bottom.visible = bottomCap.visible = false;
      }
    }
  }

  onPlayerDied(id) {
    if (this.birds[id]) {
      this.birds[id].alive = false;
      this.birds[id].sprite.material.rotation = -Math.PI / 2;
    }
  }

  destroy() {
    this.destroyed = true;
    cancelAnimationFrame(this.animId);
    window.removeEventListener('keydown', this._onKey);
    window.removeEventListener('resize', this._onResize);
    this.canvas.removeEventListener('click', this._onClick);
    this.canvas.removeEventListener('touchstart', this._onClick);
    this.renderer.dispose();
    this.scene.traverse(obj => {
      if (obj.geometry) obj.geometry.dispose();
      if (obj.material) {
        if (obj.material.map) obj.material.map.dispose();
        obj.material.dispose();
      }
    });
  }
}
